...
- CSS Modules
- Electron (shell desktop + IPC para PDF e camada local SQLite legado)
- better-sqlite3/node:sqlite (no processo Electron)
...
- `electron/services/finance-json.service.ts`: persistencia SQLite/local e migracao de JSON legado (camada Electron).
...
- O CRUD funcional do renderer hoje esta no Supabase.
- A API `window.api` permanece principalmente para exportacao de PDF (e metodos legados expostos no preload).
...
Observacao:
- No fluxo atual do renderer, esses handlers financeiros estao legados para CRUD (renderer usa Supabase).
...
### `electron/services/finance-json.service.ts`
Funcoes utilitarias e persistencia local:
- `isTransactionType(value)`
- `sanitizeTransaction(transaction)`
- `readLegacyFileData(filePath)`
- `getFinanceDirectoryPath(desktopPath)`
- `getFinanceDbPath(desktopPath)`
- `getFinanceReportsDirectoryPath(desktopPath)`
- `getLegacyJsonPathFromDbPath(dbPath)`
- `ensureDatabase(dbPath)`
- `migrateLegacyJsonIfNeeded(db, dbPath)`
- `saveTransactionToDb(dbPath, transaction)`
- `getAllTransactionsFromDb(dbPath)`
- `deleteTransactionFromDb(dbPath, id)`
- `updateTransactionInDb(dbPath, transaction)`
